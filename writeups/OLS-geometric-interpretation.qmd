...
format: 
  html:
    ...
    include-in-header:
      - text: |
         <nav><strong><a href="../index.html" style="display: inline-block; text-decoration: underline; color: black;">← Back to homepage.</a></strong></nav>
    include-after-body:
      - text: |
          <footer>
            <ul>
              <li>All writing that's actually mine <a href="https://creativecommons.org/licenses/by-sa/4.0/" title="CC BY-SA 4.0">CC BY-SA 4.0</a></li>
           </ul>
          </footer>
---
...
#### Skippable: some more random thoughts on projection and residual space.
...
The standard errors for each term in fact fall out of the estimate of a more general gadget, $Cov(\hat \beta)$. The rough idea is that, since $Cov(\hat \beta)$ are *population* variances and covariances of our estimates, the $\widehat{Cov(\hat \beta)}$ provides *sampling* variances and covariances of or estimates, and the square roots of the diagonals of $\widehat{Cov(\hat \beta)}$ are sampling standard deviations of our covariances -- that is, standard errors.

But what is $\widehat{Cov(\hat \beta)}$? Roughly, we will compute $Cov(\hat \beta)$ and "take its estimate", like so.
